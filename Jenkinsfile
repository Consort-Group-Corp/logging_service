pipeline {
  ...
  environment {
    ...
    CONTAINER_NAME = 'consort-logging-service'
    DOCKER_NETWORK = 'consort-infra_consort-network'
    LOGS_DIR       = '/app/logs/logging-service'
      ...
      steps {
        sh '''
          ...
          docker run -d \
            ...
            -p ${APP_PORT}:${APP_PORT} \
            -v ${LOGS_DIR}:/app/logs/logging-service \
            --env-file ${ENV_FILE} \
            -e TZ=Asia/Tashkent \
            ...
            -e SPRING_DATA_REDIS_HOST=consort-redis-logging-service \
